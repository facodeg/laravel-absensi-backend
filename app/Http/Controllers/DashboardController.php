...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        $lateCount = $attendances->filter(function ($item) use ($company) {
            return Carbon::parse($item->time_in)->gt(Carbon::parse($company->time_in));
        })->count();

        $earlyLeaveCount = $attendances->filter(function ($item) use ($company) {
        ...
        return view('pages.dashboard', compact('attendances', 'noteCount', 'noteData', 'izinChartData', 'company', 'izinData', 'unapprovedIzinCount', 'lateCount', 'earlyLeaveCount', 'onTimeCount'));
    }
}
